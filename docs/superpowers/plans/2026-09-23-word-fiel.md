...
## Desvios na execução (23/09/2026)

- **`stripCssUrls`** foi pra `src/lib/convert/cssUrls.ts` (módulo puro): importar do `htmlPipeline` puxava o pdf.js e o teste em Node quebrava (`DOMMatrix is not defined`).
- **Campos de página:** além do marcador, (a) campo complexo sem valor salvo ganha um `w:t` novo com o marcador e (b) run que mistura texto com `fldChar`/`instrText` é separada — a docx-preview pula a run INTEIRA quando ela tem campo (`renderRun → fieldRun`), e .docx gerado por biblioteca põe "Página " + campo numa run só (o rodapé sumia).
- **Corte de parágrafo:** o ponto de corte é o meio entre os TOPOS das linhas, não o meio do vão — com espaçamento 1,15 a caixa do texto é mais alta que a linha e as caixas se sobrepõem; o corte saía 1 linha depois e a página transbordava.
- **Espaço-depois do último bloco** de cada página vai a 0 (o corte mede a borda do bloco, sem margem; como no Word).
- **Fontes:** caminho pelo `import.meta.env.BASE_URL`; na conversão também são carregadas no documento PRINCIPAL (o html2canvas pinta num canvas de lá — sem isso as palavras saíam sobrepostas com a fonte do sistema). O Carlito foi renomeado "PDFBox Sans C" (Reserved Font Name da OFL); hinting removido (~1,04 MB no total).
- **CSS base das páginas:** desfaz o preflight do Tailwind (imagem `inline`/`baseline`, `line-height: normal`, `box-sizing` padrão, lista das notas) e desliga a hifenização automática (a lib liga `hyphens:auto`; o Word não hifeniza por padrão).
- **`DocxView`:** a escala é `transform` dentro de uma caixa com o tamanho já escalado (não `zoom` CSS): não relayouta, então as quebras medidas no palco sem escala continuam valendo; o ponto focal da pinça/toque duplo é reposto medindo a caixa depois do zoom.
- **Harness:** `vite preview` por API (sem processo solto); 32/32. Teste de "arquivo corrompido" ficou com o viewer vazio (com arquivo aberto não há botão de abrir outro — o caminho atômico é o mesmo do PDF).
- **Suíte vitest:** 175/175 com a máquina livre; com carga alta, 3–4 testes pesados de pdf.js/senha estouram os 5 s — a `main` falha nos mesmos, é ambiente.
